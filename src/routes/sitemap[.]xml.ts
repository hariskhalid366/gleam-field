...
import { services, technicians } from "@/data/servicepro";
    ...
    handlers: {
      GET: async () => {
        const entries: Entry[] = [
          ...
          { path: "/become-a-technician", changefreq: "monthly", priority: "0.7" },
          { path: "/book", changefreq: "monthly", priority: "0.8" },
          ...services.map((s) => ({ path: `/services/${s.slug}`, changefreq: "monthly", priority: "0.7" })),
          ...technicians.map((t) => ({ path: `/technicians/${t.slug}`, changefreq: "monthly", priority: "0.6" })),
        ];
